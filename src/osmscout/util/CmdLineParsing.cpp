...
CmdLineParseResult CmdLineAlternativeFlagArgParser::Parse(CmdLineScanner & /*scanner*/) {
  if (!lastArgumentCalled.empty()) {
    return CmdLineParseResult(
      "You cannot call flag argument '" + GetArgumentName() + "', since alternative flag argument '" +
      lastArgumentCalled + "' was already called");
  }
...
CmdLineParseResult CmdLineBoolArgParser::Parse(CmdLineScanner &scanner) {
  ...
  if (value == "true") {
    ...
    return CmdLineParseResult(
      "Value for boolean argument '" + GetArgumentName() + "' must be either 'true' or 'false' but not '" + value +
      "'");
  }
}
